...
void test_bench::output_p()
{
	...
	for ( int i = 0; i < values.size(); ++i )
	{
		...
		//Now wait for valid DUT output before reading.
		wait( output_valid );
...
void test_bench::sm_monitor()
{
	...
	for ( int i = 0; i < 100; ++i )
	{
		cout << "DEBUG: value1 is now holding " << hex << memory->read( 1 ) 
		<< " (iteration " << dec << i << ")" << endl;
		
		// Read only once per cycle!
		wait();
	}

	cout << "DEBUG: THE MEMORY MONITOR TERMINATED" << endl;
}