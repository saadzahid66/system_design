...
void kvazaar_ip_sub::b_transport( tlm::tlm_generic_payload& trans, sc_time& delay )
{
    ...
    {
    		printf("TLM-2 Error: Target does not support given payload transaction, %s, %s, %d\n",__FILE__,__func__,__LINE__);
    		sc_stop();
    }
    ...
    switch(adr & 0xf0000)
    {
        ...
        case LAMBDA_BASE:
        {
            CHECK_OVERFLOW(LAMBDA_BASE,RESULT_SIZE);
            if ( cmd == tlm::TLM_READ_COMMAND )
            {
        ...
        case RESULT_BASE:
        {
    		...
    		default:
    		{
      			printf("TLM-2 Error: Segmentation fault, %s, %s, %d\n",__FILE__,__func__,__LINE__);
      			sc_stop();
      			break;
    		}
    }
