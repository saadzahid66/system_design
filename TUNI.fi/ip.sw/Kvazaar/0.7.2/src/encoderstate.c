...
static void encoder_state_worker_encode_lcu(void * opaque) {
  const lcu_order_element_t * const lcu = (const lcu_order_element_t*)opaque;
  ...
  //Terminator
  if (lcu->index < (int)state->lcu_order_count - 1) {
...
static void encoder_state_encode_leaf(encoder_state_t * const state) {
  ...
  if (!use_parallel_encoding) {
    ...
    for (int i = 0; i < (int)state->lcu_order_count; ++i) {
    ...
    for (int i = 0; i < (int)state->lcu_order_count; ++i) {
      ...
      if (!state->encoder_control->sao_enable && i + 1 == (int)state->lcu_order_count) {
...
static void encoder_state_worker_encode_children(void * opaque) {
  encoder_state_t *sub_state = (encoder_state_t*)opaque;
...
static void encoder_state_worker_sao_reconstruct_lcu(void *opaque) {
  worker_sao_reconstruct_lcu_data *data = (worker_sao_reconstruct_lcu_data*)opaque;
  videoframe_t * const frame = data->encoder_state->tile->frame;
  unsigned stride = frame->width_in_lcu;
  int x;
  
  //TODO: copy only needed data
  kvz_pixel *new_y_data = (kvz_pixel*)MALLOC(kvz_pixel, frame->width * frame->height);
  kvz_pixel *new_u_data = (kvz_pixel*)MALLOC(kvz_pixel, (frame->width * frame->height) >> 2);
  kvz_pixel *new_v_data = (kvz_pixel*)MALLOC(kvz_pixel, (frame->width * frame->height) >> 2);
    ...
    if (node_is_the_last_split_in_tree) {
      ...
      if (main_state->encoder_control->sao_enable && main_state->children[0].type == ENCODER_STATE_TYPE_WAVEFRONT_ROW) {
        ...
        for (y = 0; y < frame->height_in_lcu; ++y) {
          worker_sao_reconstruct_lcu_data *data = (worker_sao_reconstruct_lcu_data*)MALLOC(worker_sao_reconstruct_lcu_data, 1);
          threadqueue_job_t *job;
#ifdef KVZ_DEBUG
          char job_description[256];
...
static void encoder_state_ref_sort(encoder_state_t *state) {
  ...
  for (int j = 0; j < (int)state->global->ref->used_size; j++) {
    if (state->global->ref->pocs[j] < state->global->poc) {
...
static void encoder_state_remove_refs(encoder_state_t *state) {
  ...
  while (check_refs || state->global->ref->used_size > (uint32_t)refnumber) {
    ...
    if (encoder->cfg->gop_len) {
      for (int ref = 0; ref < (int)state->global->ref->used_size; ref++) {
        uint8_t found = 0;
...
static void encoder_state_new_frame(encoder_state_t * const state) {
  ...
  if (state->type == ENCODER_STATE_TYPE_MAIN) {
    ...
    sqrt_lambda_temp = sqrt(lambda);
    state->global->cur_lambda_cost_sqrt = sqrt_lambda_temp;
...
#if defined(IP_ACC) || defined(SIMULATION_UNTIMED) || defined(EXPLORATION_HW)
    {
		...
		if(config)
		{
			// Calling send_lambda function implemented in encoderstate_supplement.c
			// implementation differs depending on the system design
			send_lambda(state->global->cur_lambda_cost_sqrt);
			config = 0;
		}
    }
#endif
  ...
  if (cur_cu->type == CU_INTER) {
    ...
    {  // start intra chroma pred mode coding
      ...
      if (intra_pred_mode_chroma == intra_pred_mode[0]) {
        ...
        // duplicate mode.
        for (i = 0; i < 4; ++i) {
          if (intra_pred_mode[0] == (int)chroma_pred_modes[i]) pred_mode = i;
        }
      } else {
        for (i = 0; i < 4; ++i) {
