...
videoframe_t *kvz_videoframe_alloc(const int32_t width, const int32_t height, const int32_t poc) {
  videoframe_t *frame = (videoframe_t*)MALLOC(videoframe_t, 1);
  ...
  frame->sao_luma = (sao_info_t*)MALLOC(sao_info_t, frame->width_in_lcu * frame->height_in_lcu);
  frame->sao_chroma = (sao_info_t*)MALLOC(sao_info_t, frame->width_in_lcu * frame->height_in_lcu);
...
{
  assert((int)x_in_scu < (frame->width_in_lcu << MAX_DEPTH));
  assert((int)y_in_scu < (frame->height_in_lcu << MAX_DEPTH));
