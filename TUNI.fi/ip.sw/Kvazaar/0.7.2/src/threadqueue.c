...
static void* threadqueue_worker(void* threadqueue_worker_spec_opaque) {
  threadqueue_worker_spec* threadqueue_worker_spec_temp = (threadqueue_worker_spec*)threadqueue_worker_spec_opaque;
  threadqueue_worker_spec * const threadqueue_worker_spec = threadqueue_worker_spec_temp;
  ...
  for(;;) {
    ...
    if (next_job) {
      ...
      for (i = (threadqueue->fifo ? threadqueue->queue_start : threadqueue->queue_count - 1);
           (threadqueue->fifo ? i < (int)threadqueue->queue_count : i >= (int)threadqueue->queue_start); 
    ...
    if (job) {
      ...
      for (i = 0; i < (int)job->rdepends_count; ++i) {
      ...
      assert((int)threadqueue->queue_waiting_dependency >= queue_waiting_dependency_decr);
...
int kvz_threadqueue_init(threadqueue_queue_t * const threadqueue, int thread_count, int fifo) {
  ...
  threadqueue->threads = (pthread_t*)MALLOC(pthread_t, thread_count);
  ...
  for(i = 0; i < thread_count; i++) {
    threadqueue_worker_spec *tqws = (threadqueue_worker_spec*)MALLOC(threadqueue_worker_spec, 1);
    if (tqws) {
...
static void threadqueue_free_jobs(threadqueue_queue_t * const threadqueue) {
  int i;
  for (i=0; i < (int)threadqueue->queue_count; ++i) {
...
threadqueue_job_t * kvz_threadqueue_submit(threadqueue_queue_t * const threadqueue, void (*fptr)(void *arg), void *arg, int wait, const char* const debug_description) {
  ...
  job = (threadqueue_job_t*)MALLOC(threadqueue_job_t, 1);
  ...
  if (threadqueue->queue_count >= threadqueue->queue_size) {
    threadqueue->queue = (threadqueue_job_t**)realloc(threadqueue->queue, sizeof(threadqueue_job_t *) * (threadqueue->queue_size + THREADQUEUE_LIST_REALLOC_SIZE));
...
int kvz_threadqueue_job_dep_add(threadqueue_job_t *job, threadqueue_job_t *depends_on) {
  ...
  if (depends_on->rdepends_count >= depends_on->rdepends_size) {
    depends_on->rdepends = (threadqueue_job_t**)realloc(depends_on->rdepends, sizeof(threadqueue_job_t *) * (depends_on->rdepends_size + THREADQUEUE_LIST_REALLOC_SIZE));
