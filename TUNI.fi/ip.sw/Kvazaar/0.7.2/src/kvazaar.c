...
static kvz_encoder * kvazaar_open(const kvz_config *cfg)
{
  ...
  encoder = (kvz_encoder*)calloc(1, sizeof(kvz_encoder));
  if (!encoder) {
    goto kvazaar_open_failure;
  }
  ...
  encoder->states = (encoder_state_t*)calloc(encoder->num_encoder_states, sizeof(encoder_state_t));
  if (!encoder->states) {
    goto kvazaar_open_failure;
  }
  ...
  for (int i = 0; i < (int)encoder->num_encoder_states; ++i) {
    if (i == 0) {
...
#ifdef __cplusplus
  info->nal_unit_type = (kvz_nal_unit_type)state->global->pictype;
#else
                          ...
                          kvz_frame_info *info_out)
{
  ...
  if (kvz_encoder_feed_frame(&enc->input_buffer, state, pic_in)) {
    assert(state->global->frame == (int)enc->frames_started);
    // Start encoding.
...
#ifdef __cplusplus
static kvz_api kvz_8bit_api;

const kvz_api * kvz_api_get(int bit_depth)
{
  kvz_8bit_api.config_alloc = kvz_config_alloc;
  kvz_8bit_api.config_init = kvz_config_init;
  kvz_8bit_api.config_destroy = kvz_config_destroy;
  kvz_8bit_api.config_parse = kvz_config_parse;
  
  kvz_8bit_api.picture_alloc = kvz_image_alloc;
  kvz_8bit_api.picture_free = kvz_image_free;
  
  kvz_8bit_api.chunk_free = kvz_bitstream_free_chunks;
  
  kvz_8bit_api.encoder_open = kvazaar_open;
  kvz_8bit_api.encoder_close = kvazaar_close;
  kvz_8bit_api.encoder_headers = kvazaar_headers;
  kvz_8bit_api.encoder_encode = kvazaar_encode;
  return &kvz_8bit_api;
}
