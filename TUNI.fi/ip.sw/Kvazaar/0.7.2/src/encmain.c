...
#ifdef CAMERA
  else if(strcmp(filename, "camera_high") == 0 || strcmp(filename, "camera_high_irq") == 0 || 
          strcmp(filename, "camera_low")  == 0 || strcmp(filename, "camera_low_irq")  == 0)
  {
	// Calling open_camera function implemented in camera_supplement.c
	// implementation differs depending on the system design
	return open_camera(filename);
  }
#endif
 ...
 */
 #if defined(SIMULATION_UNTIMED) || defined(EXPLORATION_HW) || defined(EXPLORATION_SW)
int kvazaar_main(int argc, char *argv[])
#else
...
#endif
{
 ...
 #if defined(IP_ACC) || defined(SIMULATION_UNTIMED) || defined(EXPLORATION_HW) || defined(EXPLORATION_SW)
  // Calling initializator function implemented in encmain_supplement.c
  // implementation differs depending on the system design
  if( initializator(encoder->cfg->acc) < 0 )
  {
	  goto exit_failure;
  }
#endif
  ...
  //Now, do the real stuff
  {
...
#if defined(EXPLORATION_HW) || defined(EXPLORATION_SW)
	start_exploration();
	lcu_limit = encoder->cfg->lcu_limit;
#else
    ...
    for (;;) {
      ...
      if (!feof(input) && (opts->frames == 0 || frames_read < (uint32_t)opts->frames || field_parity == 1) ) {
