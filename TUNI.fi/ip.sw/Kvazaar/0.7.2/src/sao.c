                                ...
                                sao_info_t *sao_left)
{
  ...
  for (edge_class = SAO_EO0; edge_class <= SAO_EO3; ++edge_class) {
    ...
    if (sum_ddistortion < sao_out->ddistortion) {
      sao_out->eo_class = (sao_eo_class)edge_class;
...
void kvz_sao_reconstruct_frame(encoder_state_t * const state)
{
  ...
  // be enough though.
  kvz_pixel *new_y_data = (kvz_pixel*)MALLOC(kvz_pixel, frame->rec->width * frame->rec->height);
  kvz_pixel *new_u_data = (kvz_pixel*)MALLOC(kvz_pixel, (frame->rec->width * frame->rec->height) >> 2);
  kvz_pixel *new_v_data = (kvz_pixel*)MALLOC(kvz_pixel, (frame->rec->width * frame->rec->height) >> 2);
