...
kvz_picture *kvz_image_alloc(const int32_t width, const int32_t height)
{
  ...
  kvz_picture *im = (kvz_picture*)MALLOC(kvz_picture, 1);
  if (!im) return NULL;
  ...
  //Allocate memory
  im->fulldata = (kvz_pixel*)MALLOC(kvz_pixel, (luma_size + 2 * chroma_size));
  if (!im->fulldata) {
    free(im);
    return NULL;
  }
                             ...
                             const unsigned height)
{
  ...
  assert((int)(x_offset + width) <= orig_image->width);
  assert((int)(y_offset + height) <= orig_image->height);
