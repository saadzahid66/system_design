...
static const struct option long_options[] = {
  ...
  { "acc",                required_argument, NULL,  0  },
  { "lcu-limit",          required_argument, NULL,  0  },
...
{
  int ok = 1;
  cmdline_opts_t *opts = (cmdline_opts_t*)calloc(1, sizeof(cmdline_opts_t));
  if (!opts) {
    ok = 0;
    goto done;
  }
...
void print_help(void)
{
  fprintf(stderr,
    ...
    "  FPGA Accelerator:\n"
    "          --acc <int>            : Specifies the ip acc support for different LCUs\n"
    "                                   to be accelerated at the same time.\n"
	"  SystemC exploration:\n"
	"          --lcu-limit <integer>  : Number of LCUs to code [all]\n"
    "\n"
