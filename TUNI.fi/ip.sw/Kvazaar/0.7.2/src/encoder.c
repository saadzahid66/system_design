...
encoder_control_t* kvz_encoder_control_init(const kvz_config *const cfg) {
  ...
  encoder = (encoder_control_t*)calloc(1, sizeof(encoder_control_t));
  if (!encoder) {
  ...
  encoder->threadqueue = (threadqueue_queue_t*)MALLOC(threadqueue_queue_t, 1);
...

  {
    ...
    encoder->tiles_col_width = tiles_col_width =
      (int32_t*)MALLOC(int32_t, encoder->tiles_num_tile_columns);
    encoder->tiles_row_height = tiles_row_height =
      (int32_t*)MALLOC(int32_t, encoder->tiles_num_tile_rows);

    encoder->tiles_col_bd = tiles_col_bd =
      (int32_t*)MALLOC(int32_t, encoder->tiles_num_tile_columns + 1);
    encoder->tiles_row_bd = tiles_row_bd =
      (int32_t*)MALLOC(int32_t, encoder->tiles_num_tile_rows + 1);

    encoder->tiles_ctb_addr_rs_to_ts = tiles_ctb_addr_rs_to_ts =
      (int32_t*)MALLOC(int32_t, num_ctbs);
    ...
    //Slices
    {
      ...
      if (encoder->slice_count == 0) {
        ...
        encoder->slice_addresses_in_ts = slice_addresses_in_ts =
          (int*)MALLOC(int, encoder->slice_count);
