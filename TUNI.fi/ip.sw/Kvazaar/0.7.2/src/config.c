...
int kvz_config_init(kvz_config *cfg)
{
  ...
  cfg->ime_algorithm   = KVZ_IME_HEXBS; /* hexbs */
  cfg->fme_level       = 1;
  ...
  cfg->owf = -1;
  cfg->slice_count = 1;
  cfg->slice_addresses_in_ts = (int32_t*)MALLOC(int32_t, 1);
...
static int parse_tiles_specification(const char* const arg, int32_t * const ntiles, int32_t** const array) {
  ...
  *array = (int32_t*)MALLOC(int32_t, *ntiles);
  if (!*array) {
...
static int parse_slice_specification(const char* const arg, int32_t * const nslices, int32_t** const array) {
  ...
  *array = (int32_t*)MALLOC(int32_t, *nslices);
  if (!*array) {
...
#define OPT(STR) (!strcmp(name, STR))
  if OPT("width")
    ...
    cfg->acc = atoi(value);
  else if OPT("lcu-limit")
    cfg->lcu_limit = atoi(value);
	...
	if(ime_algorithm == 0)
	{
		cfg->ime_algorithm = KVZ_IME_HEXBS;
	}
	else if(ime_algorithm == 1)
	{
		cfg->ime_algorithm = KVZ_IME_TZ;
	}
  }
  else if OPT("subme")
...
int kvz_config_validate(const kvz_config *const cfg)
{
  ...
  if(cfg->acc <= 0){
    fprintf(stderr, "Input error: acc must be positive\n");
    error = 1;
  }

  if(cfg->acc > 2){
    fprintf(stderr, "Input error: ip acc supports only two threads at once\n");
    error = 1;
  }
