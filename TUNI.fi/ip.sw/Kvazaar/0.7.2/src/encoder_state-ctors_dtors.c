...
static int encoder_state_config_tile_init(encoder_state_t * const state, 
  ...
  //FIXME: move them
  state->tile->frame->coeff_y = (coeff_t*)MALLOC(coeff_t, width * height);
  state->tile->frame->coeff_u = (coeff_t*)MALLOC(coeff_t, (width * height) >> 2);
  state->tile->frame->coeff_v = (coeff_t*)MALLOC(coeff_t, (width * height) >> 2);
  ...
  if (encoder->wpp) {
    ...
    state->tile->wf_jobs = (threadqueue_job_t**)MALLOC(threadqueue_job_t*, num_jobs);
...
int kvz_encoder_state_init(encoder_state_t * const child_state, encoder_state_t * const parent_state) {
  ...
  child_state->children = (encoder_state_t*)MALLOC(encoder_state_t, 1);
  ...
  if (!parent_state) {
    ...
    child_state->global = (encoder_state_config_global_t*)MALLOC(encoder_state_config_global_t, 1);
    ...
    child_state->tile = (encoder_state_config_tile_t*)MALLOC(encoder_state_config_tile_t, 1);
    ...
    child_state->slice = (encoder_state_config_slice_t*)MALLOC(encoder_state_config_slice_t, 1);
    ...
    child_state->wfrow = (encoder_state_config_wfrow_t*)MALLOC(encoder_state_config_wfrow_t, 1);
  ...
  //Create sub-encoders
  {
    ...
    while (range_start < end_in_ts && (children_allow_slice || children_allow_tile)) {
      ...
      if ((!tile_allowed || (range_end_slice >= range_end_tile)) && !new_child && slice_allowed) {
        ...
        new_child->slice = (encoder_state_config_slice_t*)MALLOC(encoder_state_config_slice_t, 1);
      ...
      if ((!slice_allowed || (range_end_slice < range_end_tile)) && !new_child && tile_allowed) {
        ...
        new_child->global = child_state->global;
        new_child->tile = (encoder_state_config_tile_t*)MALLOC(encoder_state_config_tile_t, 1);
      ...
      if (new_child) {
        child_state->children = (encoder_state_t*)realloc(child_state->children, sizeof(encoder_state_t) * (2+child_count));
        ...
        //Fix children parent (since we changed the address), except for the last one which is not ready yet
        {
          ...
          for (i = 0; child_state->children[i].encoder_control && i < child_count; ++i) {
            ...
            for (j = 0; j < (int)child_state->children[i].lcu_order_count; ++j) {
    ...
    if (children_allow_wavefront_row && child_count == 0) {
      ...
      child_state->children = (encoder_state_t*)realloc(child_state->children, sizeof(encoder_state_t) * (num_rows + 1));
      ...
      for (i=0; i < num_rows; ++i) {
        ...
        new_child->tile = child_state->tile;
        new_child->slice = child_state->slice;
        new_child->wfrow = (encoder_state_config_wfrow_t*)MALLOC(encoder_state_config_wfrow_t, 1);
    ...
    if (child_state->is_leaf) {
      ...
      child_state->lcu_order = (lcu_order_element_t*)MALLOC(lcu_order_element_t, child_state->lcu_order_count);
      assert(child_state->lcu_order);
      
      for (i = 0; i < (int)child_state->lcu_order_count; ++i) {
        lcu_id = lcu_start + i;
          ...
          if (child_state->type == ENCODER_STATE_TYPE_WAVEFRONT_ROW) {
            ...
            for (j=0; &child_state->parent->children[j] != child_state && child_state->parent->children[j].encoder_control; ++j) {
              if (child_state->parent->children[j].wfrow->lcu_offset_y == child_state->wfrow->lcu_offset_y - 1) {
                int k;
                for (k=0; k < (int)child_state->parent->children[j].lcu_order_count; ++k) {
