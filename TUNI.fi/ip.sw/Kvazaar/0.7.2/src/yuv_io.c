                ...
                kvz_picture *img_out)
{
  ...
  if ((int)input_width == img_out->width) {
      //No need to extend pixels.
#ifdef CAMERA
	  unsigned char *data = malloc(sizeof(unsigned char)*(y_size+2*uv_size));
	  // Calling camera_read_frame function implemented in camera_supplement.c
	  // implementation differs depending on the system design
      if(camera_read_frame(data,y_size,uv_size) > 0)
      {
		memcpy(img_out->y,data,y_size);
		memcpy(img_out->u,data+y_size,uv_size);
		memcpy(img_out->v,data+y_size+uv_size,uv_size);	  
      }
      else
      {
#endif
  ...
  if ((int)input_height != img_out->height) {
                ...
                unsigned output_width, unsigned output_height)
{
  ...
  for (int y = 0; y < (int)output_height; ++y) {
